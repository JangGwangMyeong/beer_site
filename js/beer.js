...
/* 뉴스 이미지 자동 슬라이드 자바스크립트 start */
let banner = {
    rollId: null,
    interval: 1, /* 이미지가 10개면 1시작 0으로 할경우 1칸 공백생김 */
    ...
    rollInit: function (newinterval) {
        ...
        //현재 배너
        let firstitem = document.querySelector('.main-news-list li');
        if (firstitem) {
            firstitem.classList.add('currentroll');
        }
        //다음 배너
        let seconditem = document.querySelectorAll('.main-news-list li')[1];
        if (seconditem) {
            seconditem.classList.add('nextroll');
        }
        //이전 배너
        document.querySelector('.main-news-list li:last-child').classList.add('prevroll');
    ...
    rollNext: function () {
        ...
        if (document.querySelector('.currentroll').nextElementSibling) {
            ...
            document.querySelector('.main-news-list li').classList.add('nextroll');
        }
    }
}

document.addEventListener('DOMContentLoaded', function () {
    banner.rollInit(5000); // 배너 롤링 몇초마다 넘길지 정하는 구간 1000=1초

});

/* 뉴스 이미지 자동 슬라이드 자바스크립트 end */
